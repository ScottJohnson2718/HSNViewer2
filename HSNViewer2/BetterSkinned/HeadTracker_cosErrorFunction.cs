...
namespace BetterSkinned
{
    ...
    public class Rotation
    {
        ...
        public bool contributed;        // true if this rotation contributed to moving the effector
    ...
    public class RotationChain
    {
        ...
        public void AffectAnimations(ref List<Bone> boneList)
        {
            ...
            // treat the right eye just as the left.  This is a hack because this isn't officially
            // a part of the chain
            Vector3 t = boneList[(int)BoneEnum.rightEye].Translation;
            Matrix m = rotationList[rotationList.Length - 1].tweakedBoneToParent;
            m.Translation = t;
            boneList[(int)BoneEnum.rightEye].SetCompleteTransform(m);
        }
...
// To figure out the shaking must check accuracy of simplified math with actual math.
// Also, this formulation is using rotations : rotation pitch, rotation heading, delta rotation pitch, delta rotation heading.
// But then we add the delta pitch to the pitch and add the delta heading to the heading!  That isn't exactly right.
// The problem is that we need to keep an angle involved in order to check joint limits.  Yet we need a small delta angle
// in order to use the small angle approximation.  Could concat delta matrices with absolute matrices and extract pitch and heading.
// Would need a matrix to Euler function that works for row matrices.
            ...
            for (rotationIndex = chain.rotationList.Length - 1; rotationIndex >= 0; --rotationIndex)
            {
                ...
                if (r.tweakable)
                {
                    Matrix worldToTweakedBone = Matrix.Invert(r.tweakedBoneToWorld);
                    Vector3 targetPointInTweakedBoneSpace = Vector3.Transform(targetPointWorldSpace, worldToTweakedBone );

                    Matrix effectorToTweakedBone = Matrix.Invert(chain.TweakedRotationToEffector(rotationIndex));
                    ...
                    aim = Track(dt, r, targetPointInTweakedBoneSpace, effectorToTweakedBone, ref moved);

                    // The angles in the joint have changed so update the forward kinematics
                    chain.ForwardKinematics();

                    // If the rotation moved the effector towards the target then remember that it contributed
                    ...
                    if (aim > .98 &&  !moved && (rotationIndex < chain.rotationList.Length - 1))  // never aim effector away from target on purpose
                    {
                        r.pitch *= 0.98f;
                        r.heading *= 0.98f;
                        //break;
        ...
        public float Track(TimeSpan dt, Rotation rotation, Vector3 targetPoint, Matrix effectorToRotatedJoint, ref bool moved)
        {
            ...
            float dpitch = 0.0f;
            float dheading = 0.0f;  // "heading" was used interchangably with "yaw" which is not correct but too busy to fix right now.
            float dAimDPitch = 0.0f, dAimDHeading = 0.0f;

            // Compute the aim (-1...1)
            // Compute the aim without any pitch or heading involved.  The target point should take care of the pitch and heading
            // since its position has been rotated into this bone's local space.
            float aim = aimFunction(0.0f, 0.0f, targetPoint, effectorToRotatedJoint);
            float newAim = aim;
            moved = false;

            // Compute the gradients given the current small correct dpitch, dheading
            gradient(dpitch, dheading, targetPoint, effectorToRotatedJoint, ref dAimDPitch, ref dAimDHeading);
            ...
            while (iterCount++ < 4)
            {
                ...
                // These new pitchDelta and headingDelta must be small enough to use the small angle approximation
                // on sin and cos.  SHould be fine just doing one frame.
                newAim = aimFunction(pitchDelta, headingDelta, targetPoint, effectorToRotatedJoint);
                if (newAim >= aim)
                {
        ...
        /// <returns></returns>
        public float aimFunction(float dpitch, float dyaw, Vector3 targetPoint, Matrix effectorToRotatedJoint)
        {
            // dpitch must be very small to use small angle approximation.  Same with dyaw.
            ...
            Vector3 translation = effectorToRotatedJoint.Translation;
             ...
             Vector3 d = targetPoint - translation;
            Vector3 v = d / d.Length();
            float aim = (m12+dyaw*(m11+dpitch*m10))*v.Z+(-dyaw*m12+m11+dpitch*m10)*v.Y+(m10-dpitch*m11)*v.X;
        ...
        /// that are going to rotate the joint (and thus the effector) towards the target. 
        /// This gradient is computed in wxMaxima and is particular to a (Up, Forward, Left) coordinates for the joints as seen in the XNA 
        /// animation skinning sample code that I'm using.
        /// </summary>
        public void gradient(float dpitch, float dyaw, Vector3 targetPoint, Matrix effectorToRotatedJoint, ref float dAimDPitch, ref float dAimDHeading)
        {
 
            float m10 = effectorToRotatedJoint.M21;
            float m11 = effectorToRotatedJoint.M22;
            float m12 = effectorToRotatedJoint.M23;
            Vector3 translation = effectorToRotatedJoint.Translation;
 
            Vector3 d = targetPoint - translation;
            Vector3 v = d / d.Length();
 
            dAimDPitch = dyaw * m10 * v.Z + m10 * v.Y - m11 * v.X;
            dAimDHeading = m11 * v.Z + dpitch * m10 * v.Z - m12 * v.Y;
